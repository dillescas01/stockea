...
# - Prepara datos y features (lags/rollings + señal semanal)
# - Benchmark multi-modelos (por defecto OFF: ExtraTrees)
# - Reentrena con todo el histórico y predice 7 días
# - Post-proceso: redondeo a entero + clip contra capacidad (esperados semana previa) + OSA
# - Guarda:
#     (1) SOLO predicciones: /1_bronze/forecast/forecast_UTEC_7d_best.xlsx
#     (2) Histórico ACUMULADO con predicciones: /2_silver/osa_hist_Tambo_UTEC_with_forecast.xlsx
# - Si el archivo silver ya existe, actualiza sin duplicar fechas.
# - Importante: OSA NO se usa como input al modelo (ni sus lags);
#               solo se recalcula para histórico y predicciones.
# ---------------------------------------------------------
...
INPUT_PATH   = "/Users/johar/Desktop/Desarrollo/proyecto_dpd/data/1_bronze/osa_hist_Tambo_UTEC.xlsx"
OUT_FORECAST_DIR = "/Users/johar/Desktop/Desarrollo/proyecto_dpd/data/1_bronze/forecast"
OUT_SILVER_DIR   = "/Users/johar/Desktop/Desarrollo/proyecto_dpd/data/2_silver"

OUT_FORECAST_XLSX = os.path.join(OUT_FORECAST_DIR, "forecast_UTEC_7d_best.xlsx")
OUT_BACKTEST_CSV  = os.path.join(OUT_FORECAST_DIR, "backtest_results_UTEC.csv")  # solo si benchmark
...
OUT_SILVER_HISTORY = os.path.join(OUT_SILVER_DIR, "osa_hist_Tambo_UTEC_with_forecast.xlsx")

# ========= CONFIG =========
H = 7           # horizonte de pronóstico
FOLDS = 8       # nº de orígenes para walk-forward (8*7 días al final) - solo benchmark
SEED = 42
...
def load_clean_df(path: str) -> pd.DataFrame:
    ...
    # numéricos base
    for c in ["productos esperados", "productos disponibles", "osa", "puntaje google", "latitud", "longitud"]:
        if c in df.columns:
    ...
    df["osa"] = (df["productos disponibles"] / df["productos esperados"] * 100).replace([np.inf, -np.inf], np.nan)
...
def build_supervised(df: pd.DataFrame, h: int = H):
    ...
    X_cols = [
        ...
        if c not in ["productos disponibles", "productos esperados", "osa", "dow"] + Y_cols
    ]
    ...
    try:
        ...
        models["LGBM"] = MultiOutputRegressor(
            LGBMRegressor(
                ...
                subsample=0.8, colsample_bytree=0.9, random_state=SEED, n_jobs=-1, verbosity=-1
            )
        )
        models["LGBM_tuned"] = MultiOutputRegressor(
            LGBMRegressor(
                ...
                reg_alpha=0.0, reg_lambda=0.0, random_state=SEED, n_jobs=-1, verbosity=-1
            )
        )
    except Exception as e:
...
def backtest_models(X_all: pd.DataFrame, Y_all: pd.DataFrame, models: dict, h: int = H, folds: int = FOLDS):
    n = len(X_all)
    if n < folds*h + 1:
        raise ValueError(f"No hay suficientes filas para {folds} ventanas de {h} días. Filas={n}")
...
def feature_importances_safely(model, X_cols: list[str]) -> pd.DataFrame | None:
    try:
        if hasattr(model, "estimators_"):  # ExtraTrees/RandomForest
            fi = np.mean([est.feature_importances_ for est in model.estimators_], axis=0)
            return pd.DataFrame({"feature": X_cols, "importance": fi}).sort_values("importance", ascending=False)
        if hasattr(model, "estimators"):
            fis = []
            for est in model.estimators_ if hasattr(model, "estimators_") else model.estimators:
...
def _next_ids(last_id: str, n: int) -> list[str] | None:
    """Genera n IDs consecutivos si el último sigue patrón TUBdddd (dígitos)."""
...
def append_forecast_to_history(base_df: pd.DataFrame, forecast_df: pd.DataFrame, silver_path: str) -> pd.DataFrame:
    """
...
def main():
    ...
    X_all, Y_all, X_cols, sup = build_supervised(df, h=H)
    ...
    if SKIP_BENCHMARK:
        ...
        res_bt, best_name, rmse_by_model = backtest_models(X_all, Y_all, models, h=H, folds=FOLDS)
    ...
    assert len(forecast_df) == H, f"Se esperaban {H} filas de forecast y se obtuvieron {len(forecast_df)}."
    ...
    if fi_df is not None:
        ...
        print("\nTop features (aprox):")
        print(pd.DataFrame(top_features).round(4).to_string(index=False))

    # Metadatos (se guardan en BRONZE/forecast)
    meta = {
        ...
        "output_backtest_csv": OUT_BACKTEST_CSV if (not SKIP_BENCHMARK and res_bt is not None) else None,
