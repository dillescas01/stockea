"""
build_gold_tiendas_7d.py
---------------------------------------------------------
Lee el histórico silver de Tambo UTEC (con forecast incluido),
...
SILVER_PATH = os.path.join(
    BASE_DIR, "data/2_silver/osa_hist_Tambo_UTEC_with_forecast.xlsx"
)
...
def load_utec_last7(silver_path: str) -> pd.DataFrame:
    ...
    # ==================================================================
    # La línea original 'if c in df.columns or True' era incorrecta.
    # Simplemente pasamos la lista 'base_cols'.
    # La función 'ensure_cols' se encargará de verificar.
    ...
    is_utec = (df.get("id", "").astype(str).str.upper() == "TUB0001") | (
        df.get("local", "")
        .astype(str)
        .str.contains("UTEC", case=False, na=False)
    )

    utec_df = df[is_utec].copy()
    if utec_df.empty:
        # Si no identifica por id/local, asumimos que TODO el silver es UTEC
        utec_df = df.copy()
...
def build_gold_tiendas():
    """
    Construye gold_tiendas_7d.xlsx a partir del silver UTEC.
    Devuelve:
        out_df (DataFrame), GOLD_PATH (str)
    """
